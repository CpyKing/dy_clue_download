...
function getIesUidList(){
    ...
    return fetch('https://leads.cluerich.com/bff/statistic/live-screen/accounts', {
      method: 'POST',
...
function getRoomIdList(iesUid){
    ...
    return fetch('https://leads.cluerich.com/bff/statistic/live-screen/room-lists', {
      method: 'POST',
      body: JSON.stringify(requestBody),
      headers: headers
    }).then(response => response.json())
...
function downloadClue(roomName, filename, roomId, startTime, endTime){
    ...
    return fetch('https://leads.cluerich.com/bff/statistic/live-screen/clue-detail', {
      method: 'POST',
      body: JSON.stringify(requestBody),
      headers: headers
    }).then(response => response.json())
...
function loopRoomIdList(iesUidList){
    getRoomIdList(iesUidList['iesUid']).then(roomIdList => {
            for (var j = 0; j < roomIdList.length; j++){
                if (isYesterday(roomIdList[j]['liveStartTime'] * 1000)){
                    var date = new Date(parseInt(roomIdList[j]['liveStartTime'], 10) * 1000);
                    var filename = iesUidList['iesName'] + '-' + date.toString()
                    downloadClue(iesUidList['iesName'], filename, roomIdList[j]['roomId'],
                                parseInt(roomIdList[j]['liveStartTime'], 10) * 1000,
                                parseInt(roomIdList[j]['liveEndTime'], 10) * 1000);
                }else{           
...
console.log('BEGIN...')
getIesUidList().then(iesUidList => {
    for (var i = 0; i < iesUidList.length; i++){
        loopRoomIdList(iesUidList[i])
        // break;
    }
})